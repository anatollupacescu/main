...
public class Cassandra {
	...
	public void insert(final String table, final String column, final String key, final String value) {
		getMutator().insert(key, table, createColumn(column, value, ss, ss));
	}
	
	public String get(final String table, final String key, final String column) throws HectorException {
		ColumnQuery<String, String, String> q = createColumnQuery(keyspace, ss, ss, ss);
	...
	public void delete(final String table, String column, String ... keys) {
		...
		for (String key : keys) {
			m.addDeletion(key, table, column, ss);
		}
		m.execute();
	}
	...
	public MultigetSliceQuery<String, String, String> getMultigetQuery(String type, String[] keys) {
		MultigetSliceQuery<String, String, String> q = HFactory.createMultigetSliceQuery(keyspace, ss, ss, ss);
		q.setColumnFamily(TABLE);
		q.setKeys(keys);
		return q;
	}
	
	public String executeQuery(Query... qs) throws XMLStreamException {
