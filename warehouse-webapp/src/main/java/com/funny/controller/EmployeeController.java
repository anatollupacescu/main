...
public class EmployeeController {
	...
	public String create(@Valid Employee employee, BindingResult result) {
		...
		logger.debug("[create] Leaving method - Saved employee");
	...
	public String getUpdateForm(@PathVariable Long id, Model model) {
		...
		if (employee == null) {
			logger.debug("[getUpdateForm] returning " + UPDATE_FORM);
			return UPDATE_FORM;
		}
		model.addAttribute(employee);
		return UPDATE_FORM;
	...
	public String update(@Valid Employee employee, BindingResult result) {
		...
		employeeService.put(employee);
		logger.debug("[update] Leaving method - Employee updated : " + Util.json(employee));
		return REDIRECT_EMPLOYEE;
	}
